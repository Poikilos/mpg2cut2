          ...
          int dmv, int mvscale)
{
  if (motion_vector_count==1)
  {
    ...
    motion_vector(PMV[0][s], dmvector,
                  h_r_size,  v_r_size,
                  dmv, mvscale, 0);
  ...
  else
  {
    ...
    motion_vector(PMV[1][s], dmvector,
                  h_r_size,  v_r_size,
                  dmv, mvscale, 0);
  }
}
...
static void motion_vector(int *PMV, int *dmvector, 
                          int h_r_size, int v_r_size,
                          int dmv, int mvscale, int full_pel_vector)
{
  ...
  decode_motion_vector(&PMV[0], h_r_size,motion_code,
                      motion_residual, full_pel_vector);
  ...
  /* vertical component */
  motion_code     = Get_motion_code();
  motion_residual = (v_r_size != 0 && motion_code !=0 ) ? 
                                     Get_Bits(v_r_size) : 0;

  if (mvscale)
      PMV[1] >>= 1; /* DIV 2 */

  decode_motion_vector(&PMV[1], v_r_size,
                       motion_code, motion_residual, full_pel_vector);
                           ...
                           int mvy)
{
  ...
  else
  {
    ...
    if (MPEG_Pic_Structure == TOP_FIELD)
      DMV[0][1]--;
    else
      DMV[0][1]++;
  }
...
static int Get_motion_code()
{
  ...
  {
    code >>= 6;
    InputBuffer_Flush(MVtab0[code].len);
  ...
  {
    code >>= 3;
    InputBuffer_Flush(MVtab1[code].len);
  ...
  if (code < 0)
  {
    ...
    {
       MParse.Fault_Flag = 10;      // Invalid motion_code
    }
    return 0;
  }


  InputBuffer_Flush(MVtab2[code].len);
