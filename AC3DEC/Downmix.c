...
double drc[256];
static double DRCScale[5] = { 1.0, 1.5, 2.0, 4.0, 8.0};
...
__forceinline short SaturateRound(double flt)
{
	...
	return (short)( (tmp<-32768) ? -32768 : ((tmp>32767) ? 32767 : tmp));
}
...
void drc_init()
{
	...
	for (i=0; i<128; i++)
		drc[i] = pow(DRCScale[AC3_DRC_FLag], i/32.0);

	for (i=128; i<256; i++)
		drc[i] = pow(DRCScale[AC3_DRC_FLag], (i-256)/32.0);
...
static void downmix_3f_2r_to_2ch(double gain, 
                                 bsi_t* bsi, 
...
static void downmix_3f_2r_to_2ch_dolby(double gain, bsi_t* bsi, 
                                       stream_samples_t samples, 
                                       sint_16 *s16_samples)
{
	uint_32 j;
	...
	if (bsi->lfeon)
	{
		...
		for (j = 0; j < 512; j += 2) 
		{
			...
			tmp = (0.3204 * (*left++ + *lfe) 
          + 0.2265 * (*centre - ds_sur)) 
          * gain;

			s16_samples[j] = SaturateRound(tmp);
		
			tmp = (0.3204 * (*right++ + *lfe++) 
          + 0.2265 * (*centre++ + ds_sur)) 
          * gain;
...
void downmix(audblk_t *audblk, bsi_t* bsi, stream_samples_t samples, sint_16 *s16_samples)
{
	...
	if (AC3_DRC_FLag>2)
		gain = 32768.0 * drc[bsi->compr]     * AC3_PreScale_Ratio;
	else
		gain = 32768.0 * drc[audblk->dynrng] * AC3_PreScale_Ratio;
	...
	switch (bsi->acmod)
	{
		...
		// 1/0, 1+1, discard dynrng2 :p    (Mono, Bi-lingual ?)
		case 1:
		case 0:
